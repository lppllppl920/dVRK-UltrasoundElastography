...
extern "C" {
    void ncc_slow(int height, int width, short int * comp_matrix_short,
            ...
            float *average_cross, float *average_strain,
            float *noise_percentage, float F0, float FS,
            int strain_or_displacement, int NOF);
    void cuda_copy_host(void *dst, void *src, size_t ptr_size);
    void dp_disparity(short int* image1_short_local,
            short int *image2_short_local, int image1_m, int image1_n,
            float w_smooth, float mu, unsigned char **strain_ret,
            int *out_height, float *average_strain, float *noise_percentage,
            int *drange_rf_local, int *drange_a_local,
            int strain_or_displacement);
...
//	prb = rf_data->prb;	
        ...
        set_threshold_values((float) fhr.ss, (float) fhr.uly, (float) fhr.ulx,
                (float) fhr.ury, (float) fhr.urx, (float) fhr.brx,
                (float) fhr.bry);
        for (burst_loop = 1; burst_loop < is_burst; burst_loop++) {
            in_queue.wait_and_pop(rf_data);
            recurring_data = rf_data->data;
            add_char_data((short int *) *data, (short int *) recurring_data,
                    height, width, is_burst);
    ...
    void execute_TRuE(cost *C, vector<data_frame_queue *> *vec_a,
            igtl::MUSiiCTCPServer::Pointer pServer, data_frame_queue *rf_data,
            int height, int width, int no_of_frames, int top_n,
            int iteration_count, int overall_iteration_count) {
