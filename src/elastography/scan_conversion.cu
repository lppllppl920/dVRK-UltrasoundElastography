...
void __global__ g_curved_scan_convert(data_type *buffer, data_type *out,
		int out_x, int out_y, int x, int y, int z, float center, float probeRad,
		float probeAng, float angStep, int start) {
...
void __global__ g_curved_scan_convert_3(data_type *buffer, data_type *out,
		int out_x, int out_y, int x, int y, int z, float center, float probeRad,
		float probeAng, float angStep, float cut_off, float ratio_x,
		float ratio_y, int start) {
	int i = blockIdx.y; //corresponds to y
...
void __global__ g_rotate_image(data_type *output, data_type *input, int x,
		int y, int z) {
	int i, j, k;
	...
	input[(x - j - 1) * y * z + k * z + i] = output[i * x * y + k * x + j];
...
data_type * cuda_average_filter(data_type *buffer, int x, int y, int z,
	...
	g_average_filter<<<blocks, threads>>>(output_d, buffer_d, x, y, z, window);
...
void __global__ g_curved_scan_convert(data_type *buffer, data_type *out,
	...
	out[i * height * width + j * width + k] = buffer[Z * x * y + Y * x + X];
}
...
void __global__ g_linear_scan_conversion(data_type *output, data_type *input,
	...
	output[i * (display_x) * (display_y) + j * (display_x) + k] = input[i * x
			* y + Y * x + X];
}
...
int cuda_linear_scan_conversion(data_type *input, data_type **output_t, int x,
	...
	*display_z = z;
	...
	g_linear_scan_conversion<<<blocks, threads>>>(output_d, input_d, x, y, z,
			*display_x, *display_y, *display_z, width, height);
...
//0.75 is the width at 0.75 x y point of the original image
	...
	*final_x = (probeRad + 0.75 * y) * sin(angStep) * spacing_y;
	...
	g_curved_scan_convert_3<<<blocks, threads>>>(buffer_d, out, new_x, new_y, x,
			y, z, center, probeRad, probeAng, angStep, cut_off, ratio_x,
			ratio_y, 0);
...
data_type * cuda_curved_scan_convert_2_old(data_type *buffer, int *out_x,
	...
	g_curved_scan_convert_2<<<blocks, threads>>>(buffer_d, out, i_x2, i_y2, x,
			y, z, center, probeRad, probeAng, angStep, 0);
...
data_type * cuda_curved_scan_convert(data_type *buffer, int *out_x, int *out_y,
	...
	g_curved_scan_convert<<<blocks, threads>>>(buffer_d, out, i_x2, i_y2, x, y,
			z, center, probeRad, probeAng, angStep, 0);
...
__device__ data_type d_linear_interpolate(double x1, double x2, double x,
        data_type a, data_type b) {
	return (data_type) round(
			fabs(x - x1) / fabs(x2 - x1) * b + fabs(x2 - x) / fabs(x2 - x1) * a);
}

__device__ data_type d_get_bilinear_sample(data_type *strain, double x,
...
__device__ data_type d_get_nearest_sample(data_type *strain, double x,
		double y, int z, int width, int height) {
...
__global__ void g_scale_image(data_type *out_strain, int new_width,
		int new_height, data_type *strain, int width, int height,
...
void cuda_scale_image_mm(data_type **out_Im1, int *out_width,
		int *out_height, data_type *Im1, int width, int height,
	...
	int output_size = sizeof(data_type) * new_width * new_height
			* no_of_frames;
	int input_size = sizeof(data_type) * width * height * no_of_frames;

	Im2 = (data_type *) malloc(output_size);
	...
	g_scale_image<<<blocks, threads>>>(Im2_d, new_width, new_height, Im1_d,
			width, height, no_of_frames, 0);
