...
void allocate_constant(int uncomp_matrix_x, int uncomp_matrix_y, int N,
        ...
        int volume_size) {
    cudaMemcpyToSymbol("uncomp_matrix_x_const", &uncomp_matrix_x, sizeof(int),
            0, cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol("uncomp_matrix_y_const", &uncomp_matrix_y, sizeof(int),
            0, cudaMemcpyHostToDevice);
    ...
    cudaMemcpyToSymbol("max_strain_const", &max_strain, sizeof(float), 0,
            cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol("num_strain_points_const", &num_strain_points,
...
void ncc_volume(int height, int width, short int * comp_matrix_short,
    ...
    *out_height = x;
    ...
    (comp_matrix_d, uncomp_matrix_d, t_d, corr_d, EI_1_d);

    checkCUDAError("Calling normalized cross corr");

    median_filter<<<uncomp_matrix_x_volume, x, 0>>>
    ...
    moving_average<<<uncomp_matrix_x_volume, x, 0>>>
    ...
    *displacement = EI_1;
    ...
    st_LSQSE <<<comp_matrix_x_volume, x-diff_kernel, 0>>>
    (EI_1_d, strain_d, diff_kernel,uncomp_matrix_x);
...
//TODO adjust standard deviation has to be changed to accomodate volume of data.
     ...
     */
    adjust_standard_deviation <<<uncomp_matrix_x_volume, x, 0>>>(strain_d, uncomp_matrix_x_volume, x, std_dev, strain_average);
...
//TODO colormap has to be changed to accomodate volume of data.
    if ((strain_max - strain_min) != 0) {
        ...
        if (strain_or_displacement == 0) {
        map_255_colormap <<<uncomp_matrix_x_volume, x, 0>>>(strain_d, strain_char_d, uncomp_matrix_x_volume, x, strain_min, strain_max - strain_min);
    } else {
    ...
    map_255_colormap <<<uncomp_matrix_x_volume, x, 0>>>(EI_1_d, strain_char_d, uncomp_matrix_x_volume, x, displacement_min, displacement_max - displacement_min);
}
...
cudaMemcpy(strain_char, strain_char_d,
    x * uncomp_matrix_x * sizeof(char) * volume_size, cudaMemcpyDeviceToHost);
...
short int *uncomp_matrix_short, float **cross_corr, float **displacement,
unsigned char **out_strain, int *out_height, float *average_cross,
float *average_strain, float *noise_percentage, float F0, float FS,
int strain_or_displacement, int NOF, int ncc_window, float ncc_overlap,
...
printf(
    "Sample points for image of size x %d y %d ncc_window %d ncc_overlap %.2f ncc_displacement %.2f : %d\n",
    uncomp_matrix_x, uncomp_matrix_y, ncc_window, ncc_overlap, ncc_displacement,
    x);
#endif
...
checkCudaErrors(
    cudaMemcpyAsync(uncomp_matrix_d, uncomp_matrix_short_h, uncomp_array_size,
            cudaMemcpyHostToDevice, stream_ncc));
...
(comp_matrix_d + k * (comp_matrix_x / NOF) * comp_matrix_y,
        ...
        t_d + k * (comp_matrix_x / NOF) * x, corr_d + k * (comp_matrix_x / NOF) * x,
...
(EI_1_d + k * (comp_matrix_x / NOF) * x, EI_1_smoothed_d + k * (comp_matrix_x / NOF) * x
...
(EI_1_smoothed_d + k * (comp_matrix_x / NOF) * x, EI_1_d + k * (comp_matrix_x / NOF) * x,
...
(EI_1_d + k * (comp_matrix_x / NOF) * x, strain_d + k * (comp_matrix_x / NOF )* x , diff_kernel,uncomp_matrix_x/NOF);
}
...
checkCudaErrors(
cudaMemcpyAsync(strain, strain_d, op_array_size, cudaMemcpyDeviceToHost,
stream_ncc));
...
find_min_max_dev(strain, uncomp_matrix_x, x, &strain_min, &strain_max, &std_dev,
&strain_average, noise_percentage);
...
for (k = 0; k < NOF; k++) {
normalizecrosscorr_jumbo <<<uncomp_matrix_x/NOF, x, 0, stream_ncc>>>
(uncomp_matrix_d + k * (uncomp_matrix_x / NOF) * uncomp_matrix_y,
        comp_matrix_d + k * (comp_matrix_x / NOF) * comp_matrix_y,
        t_d + k * (comp_matrix_x / NOF) * x, corr_d + k * (comp_matrix_x / NOF) * x,
        EI_1_d + k * (comp_matrix_x / NOF) * x);
checkCUDAError("Calling normalized cross corr");
...
st_LSQSE <<<comp_matrix_x/NOF, x-diff_kernel, 0, stream_ncc>>>
(EI_1_d + k * (comp_matrix_x / NOF) * x,
        strain_d + k * (comp_matrix_x / NOF) * x, diff_kernel,uncomp_matrix_x/NOF);
...
find_min_max_dev(strain + strain_size_float * k, uncomp_matrix_x / NOF, x,
&strain_min, &strain_max, &std_dev, &strain_average, noise_percentage);
...
adjust_standard_deviation <<<uncomp_matrix_x/NOF, x, 0, stream_ncc>>>(strain_d + strain_size_float * k, uncomp_matrix_x/NOF, x, std_dev, strain_average);
...
if (strain_or_displacement == 0) {
map_255_colormap <<<uncomp_matrix_x/NOF, x, 0, stream_ncc>>>(strain_d + strain_size_float * k, strain_char_d + strain_size_char * k ,
    uncomp_matrix_x/NOF, x, strain_min, strain_max - strain_min);
} else {
find_min_max_displacement(EI_1 + strain_size_float * k, uncomp_matrix_x / NOF,
    x, &displacement_min, &displacement_max);
	//We are using strain_char_d to save memory
    //TODO name should be changed to avoid confusions in the future.
map_255_colormap <<<uncomp_matrix_x/NOF, x, 0, stream_ncc>>>(EI_1_d + strain_size_float * k, strain_char_d + strain_size_char * k,
uncomp_matrix_x/NOF, x, displacement_min, displacement_max - displacement_min);
}
checkCUDAError("Calling map_255_colormap");
} else {
printf("Error in calculating the image k %d strain_max %f strain_min %f\n", k,
strain_max, strain_min);
}
}
...
stream_ncc);
		...
		//strain_char_d will contain displacement if strain_or_displacement != 0
cudaMemcpyAsync(strain_char, strain_char_d, x * uncomp_matrix_x * sizeof(char),
...
search_end = (int) round((*Win_S) + (5) * (*Over_S) + (*max_strain) / 4) - 1;

*correlation_size = (search_end - search_start) - (range_end - range_start) + 1;
...
if (pt_index < 4) {
*(EI_1 + uncomp_matrix_x_const * pt_index * volume_size_const + line_index) = -1
- max_strain_const / 2;
*(t + uncomp_matrix_x_const * pt_index * volume_size_const + line_index) = 0;
*(corr + uncomp_matrix_x_const * pt_index * volume_size_const + line_index) = 0;
} else {
...
search_start = round(((pt_index) * Over_S_const + 1 - max_strain_const / 2))
- 1;
...
search_end = round(
Win_S_const + (pt_index) * Over_S_const + max_strain_const / 2) - 1;

if (search_end >= (uncomp_matrix_y_const)) {
search_end = (uncomp_matrix_y_const - 1);
}

f_mean = norm_mean((uncomp_matrix + N_const * line_index + range_start),
((range_end - range_start) + 1));
t_mean = norm_mean((comp_matrix + N_const * line_index + search_start),
((search_end - search_start) + 1));

normalizecrosscorr_2((comp_matrix + N_const * line_index + search_start),
(uncomp_matrix + N_const * line_index + range_start),
((search_end - search_start) + 1), ((range_end - range_start) + 1), corrar,
f_mean, t_mean);

corrar_max_index = max_index(corrar,
((search_end - search_start) - (range_end - range_start) + 1));
if ((corrar_max_index > 0)
&& (corrar_max_index < ((search_end - search_start) - (range_end - range_start)))) {
...
*(t + pt_index * uncomp_matrix_x_const * volume_size_const + line_index) =
(float) corrar_max_index + delta2;
*(corr + pt_index * uncomp_matrix_x_const * volume_size_const + line_index) =
corrar[corrar_max_index];

if (corrar[corrar_max_index] > CROSSCORRELATION_THRESHOLD_CUDA) {
*(EI_1 + pt_index * uncomp_matrix_x_const * volume_size_const + line_index) =
*(t + pt_index * uncomp_matrix_x_const * volume_size_const + line_index)
- max_strain_const / 2;
} else {
*(EI_1 + pt_index * uncomp_matrix_x_const * volume_size_const + line_index) = 0;
}
}
}
}
...
//EI_1_d, EI_1_smoothed_d, 3, 9, x, comp_matrix_x,comp_matrix_x, x
...
*(target + width * x + y) = (temp_temp[max_median] + temp_temp[max_median - 1])
/ 2;
}
}
...
float negative_threshold_std_deviation, float negative_threshold_constant,
float positive_threshold_std_deviation, float positive_threshold_constant,
float strain_value_negative_noise, float strain_value_positive_noise) {
...
/ 1000.0);
NEGATIVE_THRESHOLD_CONSTANT = (float) (negative_threshold_constant / 1000.0);
POSITIVE_THRESHOLD_STD_DEVIATION = (float) (positive_threshold_std_deviation
/ 1000.0);
POSITIVE_THRESHOLD_CONSTANT = (float) (positive_threshold_constant / 1000.0);
STRAIN_VALUE_NEGATIVE_NOISE = (float) (strain_value_negative_noise / 10000.0);
STRAIN_VALUE_POSITIVE_NOISE = (float) (strain_value_positive_noise / 10000.0);

cudaMemcpyToSymbol("CROSSCORRELATION_THRESHOLD_CUDA",
&CROSSCORRELATION_THRESHOLD, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("NEGATIVE_THRESHOLD_STD_DEVIATION_CUDA",
&NEGATIVE_THRESHOLD_STD_DEVIATION, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("NEGATIVE_THRESHOLD_CONSTANT_CUDA",
&NEGATIVE_THRESHOLD_CONSTANT, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("POSITIVE_THRESHOLD_STD_DEVIATION_CUDA",
&POSITIVE_THRESHOLD_STD_DEVIATION, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("POSITIVE_THRESHOLD_CONSTANT_CUDA",
&POSITIVE_THRESHOLD_CONSTANT, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("STRAIN_VALUE_NEGATIVE_NOISE_CUDA",
&STRAIN_VALUE_NEGATIVE_NOISE, sizeof(float), 0, cudaMemcpyHostToDevice);
cudaMemcpyToSymbol("STRAIN_VALUE_POSITIVE_NOISE_CUDA",
&STRAIN_VALUE_POSITIVE_NOISE, sizeof(float), 0, cudaMemcpyHostToDevice);
